...
- Understanding ``ps aux`` from practical view::
    ...
    x = show you all processes, not just ones attached to a terminal.
        This will include processes such as services like crond, upowered, etc

.. note::

   To display the header of the result of ``ps aux`` only. Do ``ps aux | head -1``::

     USER          PID %CPU %MEM   SZ  RSS    TTY STAT    STIME  TIME COMMAND

   - **%CPU** The percentage of time the process has used the CPU since the process started.
   - **%MEM** The percentage of real memory used by this process.
   - **SZ** The size in 1 KB units of the core image of the process.
   - **RSS** The real-memory (resident set) size of the process (in 1 KB units).
   - **TTY** The controlling terminal for the process (ie *-* The process is not associated with a terminal.)
   - **STAT** Contains the state of the process (ie *A* Active)
   - **STIME** The starting time of the process.
   - **TIME** The total runtime for the process. The time is displayed in the format of mm:ss or mmmm:ss if the runtime reaches 100 minutes
   - **COMMAND** Contains the command name

   This should make the following result clear::
